...
\begin{itemize}
    \item \textbf{Efficiency Winner}: The \textbf{Interleaved Convolutional Hybrid (MQA + ALiBi)} achieved the lowest peak VRAM (521.3 MB) while maintaining highly competitive generation speeds (24k+ Tok/Sec), making it the optimal choice for resource-constrained edge deployments.
    \item \textbf{Accuracy Winner}: The \textbf{Pre-Attention Convolutional Hybrid} achieved the lowest perplexity (27,515.80 PPL). Explicitly filtering short-range n-grams prior to global attention allows the attention heads to focus exclusively on macro-level syntactic relationships rather than getting bogged down in low-level grammar. 
\end{itemize}
